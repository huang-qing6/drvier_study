...
static int ds18b20_reset_and_wait_ack(void){
	...
	ds18b20_udelay(480);
	gpio_direction_input(gpios[0].gpio); //切换准备接受信号

	// 等待ack 低电平传入
	while(gpio_get_value(gpios[0].gpio) && timeout--){ // 拉低
		ds18b20_udelay(1); 
	}
	...
	// ds18b20芯片会拉低60-240us左右。等待ack结束
	timeout = 300;
	while(gpio_get_value(gpios[0].gpio) && timeout--){ // 拉高
		ds18b20_udelay(1); 
	}	
...
static void ds18b20_send_cmd(unsigned char cmd){
	...
	for(i = 0; i<8; i++){
		if(cmd & (1<<i) ){/* 发送1 */
			...
			gpio_direction_output(gpios[0].gpio, 1); // 拉低 + 维持大约60us
			ds18b20_udelay(60);
			gpio_direction_output(gpios[0].gpio, 1);
		}
	}
...
static void ds18b20_read_data(unsigned char *buf){
	...
	for(i = 0; i < 8; i++){
		gpio_direction_output(gpios[0].gpio, 0);
		ds18b20_udelay(2);
		gpio_direction_input(gpios[0].gpio); //准备读数据
		ds18b20_udelay(15);
...
static void ds18b20_cal_val(unsigned char ds18b20_buf[], int result[])
{
...
static ssize_t ds18b20_read (struct file *file, char __user *buf, size_t size, loff_t *offset)
{
	...
	/* 1.启动温度转换 */
	// 1.1关中断
	spin_lock_irqsave(&ds18b20_spinlock, flag);

	// 1.2发出reset信号，等待回应
	err = ds18b20_reset_and_wait_ack();
	if(err){
		spin_unlock_irqrestore(&ds18b20_spinlock, flag);
		printk("ds18b20_reset_and_wait_ack err\n");
		return err;
	}

	// 1.3发出命令 skip rom 0xcc
	...
	// 3.7 计算CRC验证数据
	err = ds18b20_verify_crc(ker_buf);
	if(err){
		return err;
	}

	/* 4.copy_to_user */
	ds18b20_cal_val(ker_buf, result_buf); // ker to res

	err = copy_to_user(buf, result_buf, 8); // res to usr
	return 8;
}
...
static int __init ds18b20_init(void)
{
	...
	/* 注册file_operations 	*/
	major = register_chrdev(0, "100ask_ds18b20", &ds18b20_drv);  /* /dev/gpio_desc */
