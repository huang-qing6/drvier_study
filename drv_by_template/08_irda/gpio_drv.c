...
static u64 g_irda_irs_times[68];
...
static ssize_t irda_read (struct file *file, char __user *buf, size_t size, loff_t *offset)
{
	...
	err = copy_to_user(buf, ker_buf, 4);
...
static void parse_irda_datas(void){
	...
	/* 判断前导格式是不是9ms低脉冲 4.5毫秒高脉冲 */
	time = g_irda_irs_times[1] - g_irda_irs_times[0]; 
	if(time < 8000000 || time > 10000000){
		goto err;
	}

	time = g_irda_irs_times[2] - g_irda_irs_times[1]; 
	...
	for(i = 0; i < 32; i++){
		m = 3 + i * 2;
		n = m + 1;
		time = g_irda_irs_times[n] - g_irda_irs_times[m]; 
		data <<= 0;
		bits++;
		/* 时长1.69ms：1 0.56：0 */
		if(time > 1000000){
...
static irqreturn_t gpio_key_isr(int irq, void *dev_id)
{
	...
	/* 记录中断发生时刻 */
	time = ktime_get_ns();
	g_irda_irs_times[g_irda_irq_cnt] = time;

	/* 累加中断次数 */
	g_irda_irq_cnt++;

	/* 次数达标后，删除定时器，解析数据，放入buf，唤醒app */
	...
	/* 发生一次中断，重置定时器 */
	mod_timer(&gpio_desc->key_timer, jiffies + msecs_to_jiffies(100)); //完整一次数据不可能超过这个时间
	return IRQ_HANDLED;
}
...
static int __init irda_init(void)
{
	...
	device_create(gpio_class, NULL, MKDEV(major, 0), NULL, "irda"); /* /dev/100ask_gpio */
